...
@Transactional
@Repository
public class TESTRoomsDAO {
	private static final Logger log = LoggerFactory.getLogger(TESTRoomsDAO.class);
	// property constants
	...
	public static TESTRoomsDAO getFromApplicationContext(ApplicationContext ctx) {
		return (TESTRoomsDAO) ctx.getBean("RoomsDAO");
	}
	...
	public List<Rooms> findAlllv(Integer i) { 
		...
		try {
			String queryString = "select distinct r from Rooms r "
					...
					+ " left join fetch r.roomuserses ruser"
					+ " left join fetch r.customerorderses cusorder "
					+ " left join fetch r.customerorderitemses cusorderitems"
					+ " where r.position=:position order by r.id ";
...
//			Query queryObject = getCurrentSession().createQuery(queryString).setString("no", "%"+a+"%");			
	...
	public Rooms findById(Integer roomId, boolean b) {
		String hql="select distinct r from Rooms r "
				...
				+ " where r.id=:id  and  "
				+ " o.orderstatus=1 or o.orderstatus=3";
	...
	public Rooms findById1(Integer roomId, boolean b) {
		String hql="from Rooms r where r=:roomId ";
				
		return (Rooms) getCurrentSession().createQuery(hql).
				setInteger("roomId", roomId).uniqueResult();
	}
